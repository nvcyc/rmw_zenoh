...
// For added rcl_buffer relevant logging, to be removed when rcl_buffer is finalized
#define RMW_ZENOH_RCL_BUFFER_LOG_INFO_NAMED(...) {rmw_zenoh_cpp::Logger::get().log_named( \
      RCUTILS_LOG_SEVERITY_INFO, __func__, __FILE__, __LINE__, __VA_ARGS__);}
#define RMW_ZENOH_RCL_BUFFER_LOG_ERROR_NAMED(...) {rmw_zenoh_cpp::Logger::get().log_named( \
      RCUTILS_LOG_SEVERITY_ERROR, __func__, __FILE__, __LINE__, __VA_ARGS__);}
